...
The setup wizard asks you to pick a theme preset, backend provider, features, and development workflow. It strips everything you don't select — unneeded feature directories, unused dependencies, provider files — and commits a clean project.
...
The setup wizard can generate AI agent workflow files tailored to your selected features. Choose between:

- **Manual** — minimal `CLAUDE.md` with tech stack and commands only
- **Agentic** — full `CLAUDE.md` with coding conventions, feature-specific guidance, `.claude/settings.json` with pre-configured permissions, and selectable slash commands

### Generated Slash Commands

| Command | Description |
| --- | --- |
| `/add-feature` | Scaffold a new feature module following the Basekit pattern |
| `/new-screen` | Create a new route/screen with proper layout integration |
| `/add-provider` | Add a new provider implementation to an existing feature |
| `/run-checks` | Lint + typecheck + test in one go |
| `/new-component` | Scaffold a component with NativeWind and theme support |
| `/create-issue` | Create a GitHub issue |
| `/create-branch` | Create a branch following `ft/`, `fix/`, `refactor/` naming |
| `/evaluate` | Scan the codebase and regenerate workflow files to match current state |

The `/evaluate` command is self-updating — as you add features, components, or providers, run it to keep `CLAUDE.md` and commands in sync with your actual codebase.

### YAML Config with Workflows
...
> Currently supports Claude Code. Cursor, Gemini, and other agent workflows are planned.
...
For teams standardizing project setup across multiple apps, see the YAML example in the [Agentic Workflows](#agentic-workflows) section above.
...
.claude/            # Generated by agentic workflow (if selected)
  settings.json     #   Pre-configured permissions
  commands/         #   Slash commands (/add-feature, /evaluate, etc.)
src/
